...
export type RemovePhotoResult = { ok: true } | { ok: false; reason: "not_found" | "forbidden" };

/** O id do arquivo dentro do link do Drive gravado na planilha (".../d/{fileId}"). */
function fileIdDaUrl(raw: string): string {
  return /\/d\/([\w-]+)/.exec(raw)?.[1] ?? "";
}

/**
 * Desfaz uma foto: manda o arquivo pro lixo do Drive e devolve as colunas de imagem ao estado
 * anterior (vazias, ou o que `restaurar` mandar). É o rollback de "anexei a foto e cancelei" —
 * antes disso o arquivo ficava no Drive pra sempre mesmo quando o item nunca chegou a ser salvo.
 * A exclusão no Drive continua passando por `arquivoDoUsuario` no Codigo.gs, que confere que o
 * arquivo está na pasta deste usuário; se ela falhar (acervo antigo enviado por outra conta, ver
 * MIGRACAO_SHEETS.md), o rollback das colunas acontece do mesmo jeito — um arquivo órfão no Drive
 * é menos grave que a planilha apontando pra uma foto que o usuário desfez.
 */
export async function removeItemPhoto(
  ...
  const fileId = fileIdDaUrl(String(row[ITEM_IMG_URL_COL[tipo]] ?? ""));
  if (fileId) {
    try {
      // Aqui as retentativas padrão VALEM (ao contrário do upload): mandar pro lixo duas vezes é
      // inofensivo, e o Apps Script devolve uma página de erro do Google de vez em quando mesmo
      // tendo executado - visto neste mesmo teste em 2026-09-03.
      await callAppsScript("driveDeleteFile", {
        fileId,
        categoria: ITEM_DRIVE_CATEGORY[tipo],
        userId: sessionUserId,
      });
    } catch (err) {
      console.error(`removeItemPhoto: não deu pra apagar ${fileId} do Drive`, err);
    }
  }
  ...
  await callAppsScript("updateById", {
    ...
    id,
    patch: {
      [ITEM_IMG_URL_COL[tipo]]: restaurar?.url ?? "",
      [ITEM_IMG_NOME_COL[tipo]]: restaurar?.nome ?? "",
      updated_at: nowIso(),
    },
  });
